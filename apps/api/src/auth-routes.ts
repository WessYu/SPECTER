...
async function fetchGitHubUser(accessToken: string): Promise<GitHubUser> {
  ...
  if (typeof body.id !== "number" || typeof body.login !== "string" || !body.login)
...
async function ensureUserWorkspace(
  prisma: PrismaClient,
  github: GitHubUser,
): Promise<{ readonly user: UserRow; readonly membership: MembershipRow }> {
  ...
  const user = (await prisma.user.upsert({
    ...
    create: {
      ...
      ...(github.avatar_url ? { avatarUrl: github.avatar_url } : {}),
    },
    update: {
      name: github.name?.trim() || github.login,
      ...(github.avatar_url ? { avatarUrl: github.avatar_url } : {}),
...
export function registerAuthRoutes(app: FastifyInstance, prisma: PrismaClient): void {
  app.get(
    ...
    async (_request, reply) => {
      ...
      reply.header(
        "set-cookie",
        serializeCookie(OAUTH_STATE_COOKIE_NAME, state, OAUTH_STATE_TTL_SECONDS, oauthStatePath()),
      );
  ...
  app.get<{
    Querystring: { readonly code?: string; readonly state?: string; readonly error?: string };
  }>(
    ...
    async (request, reply) => {
      ...
      if (request.query.error)
        return reply
          .code(302)
          .header("location", `${dashboardUrl()}/login?error=github_denied`)
          .send();
      if (
        !request.query.code ||
        !request.query.state ||
        !expectedState ||
        !constantTimeStringMatch(request.query.state, expectedState)
      ) {
  ...
  app.get("/api/v1/auth/me", {}, async (request, reply) => {
    ...
    if (auth.kind !== "session")
      return reply.send({ kind: "api-key", organizationId: auth.organizationId });
    ...
    return membership
      ? reply.send({ kind: "session", ...membership })
    ...
    if (auth.kind === "session") {
      ...
      if (rawToken)
        await prisma.session.updateMany({
          where: { tokenHash: hashOpaqueToken(rawToken), userId: auth.principalId },
  ...
  app.post<{ Params: OrgParams }>(
    "/api/v1/auth/organizations/:id/activate",
    {},
    async (request, reply) => {
      const auth = getAuth(request);
      if (auth.kind !== "session") return reply.code(403).send({ error: "session_required" });
      const membership = await prisma.organizationMember.findFirst({
        where: { userId: auth.principalId, organizationId: request.params.id },
        select: { role: true },
      });
  ...
  app.post<{ Params: ApiKeyParams }>(
    ...
    async (request, reply) => {
      ...
      const updated = await prisma.apiKey.updateMany({
        where: { id: request.params.id, organizationId: auth.organizationId, revokedAt: null },
        data: { revokedAt: new Date() },
      });
