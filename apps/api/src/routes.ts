...
export function registerRoutes(app: FastifyInstance, prisma: PrismaClient): void {
  ...
  app.get<{ Params: IdParams }>("/api/v1/scans/:id", {}, async (request, reply) => {
    ...
    const scan = await prisma.scan.findFirst({
      where: { id: request.params.id, project: { organizationId: auth.organizationId } },
      include: { occurrences: { include: { finding: true } } },
    });
  ...
  app.get<{ Params: IdParams }>("/api/v1/findings/:id", {}, async (request, reply) => {
    ...
    const finding = await prisma.finding.findFirst({
      where: { id: request.params.id, project: { organizationId: auth.organizationId } },
      include: { occurrences: { orderBy: { observedAt: "desc" }, take: 20 } },
    });
    return finding ? reply.send(finding) : reply.code(404).send({ error: "finding_not_found" });
  });

  app.post<{ Body: SuppressionBody }>(
    "/api/v1/suppressions",
    { schema: { body: suppressionBodySchema } },
    async (request, reply) => {
      const auth = getAuth(request);
      if (auth.kind !== "session" || !requireRole(auth, ["owner", "admin", "member"]))
        return reply.code(403).send({ error: "forbidden" });
      if (!(await requireProject(prisma, auth.organizationId, request.body.projectId)))
        return reply.code(404).send({ error: "project_not_found" });
  ...
  app.post<{ Params: IdParams; Body: { readonly action?: "create" | "check" } }>(
    "/api/v1/domains/:id/verify",
    {
      ...
      schema: {
        body: {
          ...
          properties: { action: { enum: ["create", "check"] } },
        },
      },
    },
    async (request, reply) => {
      ...
      const domain = (await prisma.domain.findFirst({
        where: { id: request.params.id, project: { organizationId: auth.organizationId } },
        select: { id: true, hostname: true, project: { select: { organizationId: true } } },
      })) as DomainRow | null;
      ...
      try {
        ...
        matched = records.some(
          (record) =>
            record.startsWith("specter-verification=") &&
            sha256(record.slice("specter-verification=".length)) === verification.tokenHash,
        );
      } catch {
        /* DNS method optional */
      }
      if (!matched) {
        try {
          const response = await safeGet(
            `https://${domain.hostname}/.well-known/specter-verification.txt`,
            {
          ...
          matched =
            response.status === 200 && sha256(response.body.trim()) === verification.tokenHash;
        } catch {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.domainVerification.update({
