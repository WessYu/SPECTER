...
const MODULE_STATUSES = new Set<ScanModuleResult["status"]>(["passed", "warning", "failed", "skipped"]);
...
function string(value: unknown, name: string, max = MAX_STRING): string {
  if (typeof value !== "string" || value.length === 0 || value.length > max) throw new Error(`${name} must be a non-empty string up to ${max} characters.`);
  return value;
}
function finite(value: unknown, name: string, min = Number.NEGATIVE_INFINITY, max = Number.POSITIVE_INFINITY): number {
...
function validateFinding(value: unknown, index: number): Finding {
  ...
  const locationRow = row.location === undefined ? undefined : record(row.location, `findings[${index}].location`);
  const locationFile = locationRow ? optionalString(locationRow.file, `findings[${index}].location.file`, 2_048) : undefined;
  const locationUrl = locationRow ? optionalString(locationRow.url, `findings[${index}].location.url`, 4_096) : undefined;
  const location = locationRow ? {
    ...(locationFile ? { file: locationFile } : {}),
    ...(locationRow.line !== undefined ? { line: integer(locationRow.line, `findings[${index}].location.line`, 1) } : {}),
    ...(locationRow.column !== undefined ? { column: integer(locationRow.column, `findings[${index}].location.column`, 1) } : {}),
    ...(locationUrl ? { url: locationUrl } : {}),
  } : undefined;
  const remediation = optionalString(row.remediation, `findings[${index}].remediation`, 8_192);
  const documentationUrl = optionalString(row.documentationUrl, `findings[${index}].documentationUrl`, 4_096);
  const status = row.status === undefined ? undefined : enumValue(row.status, FINDING_STATUSES, `findings[${index}].status`);
  return {
    schemaVersion: "1",
    ...
    ...(status ? { status } : {}),
    ...(row.firstDetectedAt !== undefined ? { firstDetectedAt: dateTime(row.firstDetectedAt, `findings[${index}].firstDetectedAt`) } : {}),
    ...(row.lastDetectedAt !== undefined ? { lastDetectedAt: dateTime(row.lastDetectedAt, `findings[${index}].lastDetectedAt`) } : {}),
  };
}

function validateModules(value: unknown): readonly ScanModuleResult[] {
  if (!Array.isArray(value) || value.length > MAX_MODULES) throw new Error(`modules must be an array with at most ${MAX_MODULES} items.`);
...
function validateErrors(value: unknown): readonly ScanError[] {
  if (!Array.isArray(value) || value.length > MAX_ERRORS) throw new Error(`errors must be an array with at most ${MAX_ERRORS} items.`);
  return value.map((item, index) => {
    const row = record(item, `errors[${index}]`);
    const moduleName = optionalString(row.module, `errors[${index}].module`, 200);
    if (typeof row.recoverable !== "boolean") throw new Error(`errors[${index}].recoverable must be boolean.`);
    return {
...
function validateSurface(value: unknown): ScanResult["surface"] {
  ...
  if (!Array.isArray(root.routes) || root.routes.length > MAX_ROUTES) throw new Error(`surface.routes must contain at most ${MAX_ROUTES} routes.`);
  if (!Array.isArray(root.externalDomains) || root.externalDomains.length > MAX_DOMAINS) throw new Error(`surface.externalDomains must contain at most ${MAX_DOMAINS} domains.`);
  const routes = root.routes.map((item, index) => {
    const row = record(item, `surface.routes[${index}]`);
    const contentType = optionalString(row.contentType, `surface.routes[${index}].contentType`, 500);
    ...
    return {
      ...
      ...(row.status !== undefined ? { status: integer(row.status, `surface.routes[${index}].status`, 0, 999) } : {}),
      ...(contentType ? { contentType } : {}),
      ...(typeof row.authenticationObservable === "boolean" ? { authenticationObservable: row.authenticationObservable } : {}),
      ...(cors ? { cors } : {}),
    };
  });
  const classifications = new Set(["first-party", "known-third-party", "unknown", "newly-introduced"] as const);
  const externalDomains = root.externalDomains.map((item, index) => {
    const row = record(item, `surface.externalDomains[${index}]`);
    if (!Array.isArray(row.resourceTypes) || row.resourceTypes.length > 100) throw new Error(`surface.externalDomains[${index}].resourceTypes is invalid.`);
    const firstOccurrence = optionalString(row.firstOccurrence, `surface.externalDomains[${index}].firstOccurrence`, 4_096);
    const page = optionalString(row.page, `surface.externalDomains[${index}].page`, 4_096);
    const relatedScript = optionalString(row.relatedScript, `surface.externalDomains[${index}].relatedScript`, 4_096);
    return {
      domain: string(row.domain, `surface.externalDomains[${index}].domain`, 253).toLowerCase(),
      classification: enumValue(row.classification, classifications, `surface.externalDomains[${index}].classification`),
      resourceTypes: row.resourceTypes.map((entry, itemIndex) => string(entry, `surface.externalDomains[${index}].resourceTypes[${itemIndex}]`, 100)),
      ...(firstOccurrence ? { firstOccurrence } : {}),
      ...(page ? { page } : {}),
      ...(relatedScript ? { relatedScript } : {}),
    };
  });
  const removedDomains = root.removedDomains === undefined ? undefined : (() => {
    if (!Array.isArray(root.removedDomains) || root.removedDomains.length > MAX_DOMAINS) throw new Error("surface.removedDomains is invalid.");
    return root.removedDomains.map((entry, index) => string(entry, `surface.removedDomains[${index}]`, 253).toLowerCase());
  })();
...
export function validateScanResult(value: unknown): ScanResult {
  ...
  if (!Array.isArray(findingsRaw) || findingsRaw.length > MAX_FINDINGS) throw new Error(`findings must be an array with at most ${MAX_FINDINGS} items.`);
