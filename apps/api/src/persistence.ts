...
interface SuppressionRow { readonly ruleId: string | null; readonly fingerprint: string | null; readonly reason: string; readonly expiresAt: Date | null; }
...
export async function persistScan(prisma: PrismaClient, organizationId: string, projectId: string, result: ScanResult): Promise<void> {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.scan.create({ data: {
      ...
      errorsJson: result.errors, ...(result.surface ? { surfaceJson: result.surface } : {}), startedAt: new Date(result.startedAt), completedAt: now, durationMs: Math.round(result.durationMs),
    } });

    const observedFingerprints: string[] = [];
    for (const finding of result.findings) {
      observedFingerprints.push(finding.fingerprint);
      const suppressed = finding.status === "suppressed" || suppressions.some((item) => matchesSuppression(finding, item, now));
      ...
      for (const domain of result.surface.externalDomains) {
        await tx.externalDomain.upsert({
          ...
          create: {
            ...
            resourceTypes: [...domain.resourceTypes], firstSeenAt: now, lastSeenAt: now,
          },
          update: {
            scanId: result.scanId, classification: domain.classification, resourceTypes: [...domain.resourceTypes], lastSeenAt: now,
          },
        });
      }
...
async function persistFinding(tx: PrismaClient, projectId: string, result: ScanResult, finding: Finding, status: "open" | "suppressed"): Promise<void> {
  ...
  const record = await tx.finding.upsert({
    ...
    create: {
      ...
      firstDetectedAt: observedAt, lastDetectedAt: observedAt, status,
    },
    update: {
      title: finding.title, description: finding.description, severity: finding.severity, confidence: finding.confidence,
      lastDetectedAt: observedAt, status, ...(finding.remediation ? { remediation: finding.remediation } : {}),
    }, select: { id: true },
  }) as FindingRow;
  await tx.findingOccurrence.upsert({
    ...
    update: {
      severity: finding.severity, observedAt,
      ...(finding.location?.file ? { file: finding.location.file } : {}), ...(finding.location?.line !== undefined ? { line: finding.location.line } : {}),
      ...(finding.location?.column !== undefined ? { column: finding.location.column } : {}), ...(finding.location?.url ? { url: finding.location.url } : {}),
      ...(finding.evidence !== undefined ? { evidenceJson: redactEvidence(finding.evidence) } : {}),
    },
