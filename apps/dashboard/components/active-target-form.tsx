...
import { createActiveTarget, type ActiveVerificationState } from "../app/(app)/actions";

const initial: ActiveVerificationState = {};

export function ActiveTargetForm({ projectId }: { readonly projectId: string }) {
  const [state, action, pending] = useActionState(createActiveTarget, initial);
  return (
    <div>
      ...
      {state.content ? (
        <div className="key-reveal" role="status">
          ...
          <p className="subtle">
            Publish this exact content on the target hostname. The token is shown here once and is
