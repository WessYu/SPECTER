import type { Finding, ScanDiff, ScanResult, SecurityRegression, Severity, SeverityChange } from "@specter/types";
...
export function compareScans(previous: ScanResult, current: ScanResult): ScanDiff {
  ...
  for (const finding of current.findings) {
    ...
    if (before.severity !== finding.severity) {
      severityChanged.push({ fingerprint: finding.fingerprint, previous: before.severity, current: finding.severity, finding });
    } else unchanged.push(finding);
  }

  for (const finding of previous.findings) if (!currentByFingerprint.has(finding.fingerprint)) resolved.push(finding);

  const bySeverityThenFingerprint = (a: Finding, b: Finding) => rank[b.severity] - rank[a.severity] || a.fingerprint.localeCompare(b.fingerprint);
  ...
  severityChanged.sort((a, b) => rank[b.current] - rank[a.current] || a.fingerprint.localeCompare(b.fingerprint));
...
export function evaluateRegression(previous: ScanResult, current: ScanResult): SecurityRegression {
  ...
  const newHighOrCritical = diff.new.filter((finding) => finding.severity === "high" || finding.severity === "critical").length;
  const severityIncreases = diff.severityChanged.filter((change) => rank[change.current] > rank[change.previous]).length;
  return {
