...
function scan(id: string, score: number, findings: readonly Finding[]): ScanResult {
  return {
    schemaVersion: "1",
    scanId: id,
    target: { kind: "project", value: "/tmp/app" },
    startedAt: "2026-01-01T00:00:00.000Z",
    completedAt: "2026-01-01T00:00:01.000Z",
    durationMs: 1000,
    status: "completed",
    score: { value: score, band: score >= 75 ? "good" : "fair", deductions: [] },
    summary: { info: 0, low: 0, medium: 0, high: findings.length, critical: 0 },
    findings,
    modules: [],
    errors: [],
  };
...
describe("core security primitives", () => {
  ...
  it("redacts nested sensitive values", () => {
    expect(
      redactEvidence({
        token: "spt_test_Q7m9Z2x8N4v6K1r5T3w0",
        secret: "spt_test_Q7m9Z2x8N4v6K1r5T3w0",
      }),
    ).toEqual({ token: "[REDACTED]", secret: "[REDACTED]" });
  });
  ...
  it("applies explicit suppressions and ignores expired ones", () => {
    ...
    const active = applySuppressions(
      [finding],
      [{ ruleId: rule.id, reason: "accepted risk" }],
      new Date("2026-01-01"),
    );
    ...
    const expired = applySuppressions(
      [finding],
      [{ ruleId: rule.id, reason: "expired risk", expiresAt: "2025-01-01T00:00:00.000Z" }],
      new Date("2026-01-01"),
    );
  ...
  it("detects regression and fails a new-high gate", () => {
    ...
    expect(gate.failures.map((item) => item.code)).toEqual(
      expect.arrayContaining(["NEW_FINDING", "SCORE_DROP"]),
    );
  });
});
