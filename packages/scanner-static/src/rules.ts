...
const metadata = {
  eval: {
    ...
    description:
      "Direct eval() execution can turn attacker-controlled strings into executable JavaScript.",
    category: "source",
    defaultSeverity: "high",
    defaultConfidence: "high",
    remediation: "Replace eval() with explicit parsing or a constrained operation map.",
    falsePositiveGuidance:
      "Review generated-code tooling separately; runtime application code should rarely need eval().",
  },
  newFunction: {
    id: "SPECTER-SOURCE-002",
    title: "Dynamic code execution with Function constructor",
    description:
      "new Function() compiles strings as JavaScript and can create code-injection paths.",
    category: "source",
    defaultSeverity: "high",
    defaultConfidence: "high",
  ...
  dangerousHtml: {
    ...
    description:
      "dangerouslySetInnerHTML bypasses React escaping and requires trusted, sanitized HTML.",
    category: "source",
    defaultSeverity: "medium",
    defaultConfidence: "medium",
    remediation:
      "Avoid raw HTML when possible; otherwise sanitize with a proven allow-list sanitizer.",
  },
  documentWrite: {
  ...
  localStorageToken: {
    ...
    description:
      "Tokens in localStorage are readable by injected JavaScript and increase XSS impact.",
    category: "source",
    defaultSeverity: "medium",
    defaultConfidence: "medium",
  ...
  plainHttp: {
    ...
    description:
      "A non-local HTTP endpoint can expose data or create mixed-content behavior in production.",
  ...
  clientExposure: {
    ...
    description:
      "A private credential-like environment variable appears in code that may execute in the browser.",
    category: "client-exposure",
    defaultSeverity: "high",
    defaultConfidence: "medium",
    remediation:
      "Keep private environment variables in server-only modules and verify generated bundles.",
  },
...
function lexicalFindings(file: SourceFile): Finding[] {
  ...
  for (let index = 0; index < tokens.length - 2; index += 1) {
    ...
    if (current?.value === "document" && next?.value === "." && after?.value === "write") {
      result.push(
        createFinding({
          metadata: metadata.documentWrite,
          source: "static",
          location: { file: file.path, line: current.line, column: current.column },
...
function patternFindings(file: SourceFile): Finding[] {
  ...
  if (isClientFacing(file)) {
    const privateProcessEnv =
      /process\.env\.(?:DATABASE_URL|PRIVATE_KEY|SECRET_KEY|STRIPE_SECRET_KEY|INTERNAL_API_TOKEN)\b/g;
    for (const match of file.content.matchAll(privateProcessEnv)) {
      result.push(
        createFinding({
          metadata: metadata.clientExposure,
          source: "static",
          location: { file: file.path, line: lineOf(file.content, match.index) },
...
export const sourceRuleMetadata: readonly RuleMetadata[] = Object.values(metadata).map((rule) => ({
  ...rule,
}));
