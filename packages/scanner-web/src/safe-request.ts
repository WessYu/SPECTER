...
function normalizeHeaders(headers: http.IncomingHttpHeaders): Readonly<Record<string, string | readonly string[]>> {
  const output: Record<string, string | readonly string[]> = {};
  for (const [key, value] of Object.entries(headers)) if (value !== undefined) output[key.toLowerCase()] = value;
  return output;
}

function once(target: Awaited<ReturnType<typeof resolvePublicTarget>>, options: SafeRequestOptions): Promise<{ response: SafeResponse; location?: string }> {
  return new Promise((resolve, reject) => {
    ...
    const req = client.request({
      ...
      incoming.on("data", (chunk) => {
        ...
        if (bytes > max) { incoming.destroy(new Error(`Response exceeded ${max} bytes.`)); return; }
        chunks.push(chunk);
      });
      ...
      incoming.on("end", () => {
        ...
        resolve({
          response: { url: target.url.toString(), status, headers, body: Buffer.concat(chunks).toString("utf8"), bytes, redirects: [] },
          ...(location ? { location } : {}),
        });
      });
    });
    req.setTimeout(options.requestTimeoutMs ?? 10_000, () => req.destroy(new Error("Request timed out.")));
    req.on("error", reject);
    if (options.signal) {
      if (options.signal.aborted) req.destroy(new Error("Request aborted."));
      else options.signal.addEventListener("abort", () => req.destroy(new Error("Request aborted.")), { once: true });
    }
    req.end();
  });
}

export async function safeGet(input: string, options: SafeRequestOptions = {}): Promise<SafeResponse> {
