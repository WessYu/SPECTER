...
const rules = {
  sourceMap: {
    ...
    description:
      "A JavaScript source map is present in generated artifacts and may expose original source when publicly deployed.",
    category: "build",
    defaultSeverity: "medium",
    defaultConfidence: "high",
    remediation:
      "Do not publish private source maps publicly; upload them only to trusted error-monitoring systems when required.",
  },
  envFile: {
    ...
    defaultConfidence: "high",
    remediation:
      "Remove environment files from client/static output and rotate any exposed credentials.",
  },
  secret: {
    ...
    defaultConfidence: "high",
    remediation:
      "Remove the private value from browser code, rotate the credential and verify a clean rebuild.",
  },
  privateUrl: {
    id: "SPECTER-BUILD-004",
    title: "Private/internal URL present in build output",
    description:
      "Generated client artifacts contain a private-network or internal hostname reference.",
    category: "client-exposure",
    defaultSeverity: "medium",
    defaultConfidence: "medium",
    remediation:
      "Keep internal endpoints server-side and expose only intended public API origins to browser code.",
  },
  debugInfo: {
    id: "SPECTER-BUILD-005",
    title: "Debug or stack-trace information in build output",
    description:
      "Generated artifacts contain debug markers or stack traces that may reveal implementation details.",
    category: "build",
    defaultSeverity: "low",
    defaultConfidence: "medium",
    remediation:
      "Disable production debug output and ensure error responses/logging do not expose stack traces to clients.",
  },
...
export async function locateBuildOutput(
  root: string,
...
export interface TrustedBuildResult {
...
export function executeTrustedBuild(
  root: string,
  command: readonly [string, ...string[]],
  timeoutMs = 120_000,
): Promise<TrustedBuildResult> {
...
const SECRET_PATTERN =
  /\b(?:sk_live_[A-Za-z0-9]{16,}|gh[pousr]_[A-Za-z0-9_]{20,}|AKIA[0-9A-Z]{16}|(?:postgres|postgresql|mysql|mongodb(?:\+srv)?):\/\/[^\s"'`]+)/g;
const GENERIC_SECRET_ASSIGNMENT =
  /\b(?:token|secret|password|api[_-]?key|private[_-]?key)\s*[:=]\s*["'`]([^"'`\s]{20,})["'`]/gi;
const PRIVATE_URL =
  /\bhttps?:\/\/(?:localhost|127\.0\.0\.1|10\.\d+\.\d+\.\d+|192\.168\.\d+\.\d+|172\.(?:1[6-9]|2\d|3[01])\.\d+\.\d+|[A-Za-z0-9.-]+\.internal)(?::\d+)?[^\s"'`]*/gi;
...
export async function scanBuild(
  ...
  const outputs =
    options.outputDirectories?.map((item) => path.resolve(absoluteRoot, item)) ??
    (await locateBuildOutput(absoluteRoot, framework));
  ...
  async function visit(directory: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        /* unreadable artifacts are ignored but never executed */
      }
    }
  }
  ...
  return {
    framework,
    outputs: outputs.map((item) => path.relative(absoluteRoot, item).replaceAll(path.sep, "/")),
    filesScanned,
    findings,
    skippedLargeFiles,
  };
