...
export async function doctor(
  ...
  const checks = [
    [
      "Node.js >= 20",
      Number(process.version.slice(1).split(".")[0]) >= 20,
    ],
    [
      "package.json",
      await fileExists(
    ...
    [
      "Active scanner AbortController",
      typeof AbortController === "function",
    ],
    [
      "Active scanner OpenSSL",
      typeof process.versions.openssl === "string" &&
        process.versions.openssl.length > 0,
    ],
    [
      "Active scanner URL support",
      typeof URL === "function",
    ],
  ] as const;
