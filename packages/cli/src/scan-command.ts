...
function parseScanArgs(args: readonly string[]): ParsedScanArgs {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (target)
      throw new Error("Only one scan target may be supplied.");
    target = arg;
  }
...
function validateLoadedScan(
  value: unknown,
  source: string,
): ScanResult {
  if (!value || typeof value !== "object")
    throw new Error(`Invalid scan report: ${source}`);
  ...
  if (
    ...
    throw new Error(
      `Unsupported or invalid SPECTER scan report: ${source}`,
    );
...
async function persistReport(
  ...
  format: ParsedScanArgs["format"],
  cwd: string,
): Promise<string> {
  const extension = format === "sarif" ? ".sarif" : ".json";
  let file: string;
  if (!requested)
    file = path.join(cwd, ".specter", `report${extension}`);
  else {
    const absolute = path.resolve(cwd, requested);
    file = /\.(?:json|sarif)$/i.test(absolute)
      ? absolute
      : path.join(absolute, `specter${extension}`);
  }
  ...
  await writeFile(
    file,
    renderReport(scan, machineFormat),
    "utf8",
  );
  return file;
}
  ...
  try {
    ...
    const active = await runActiveScan(activeTarget, {
      config: {
        ...
        enabled: true,
        profile:
          parsed.activeProfile ?? config.active.profile,
      },
      storePath: defaultAuthorizationStore(cwd),
      ...(base.surface?.routes
        ? { observedRoutes: base.surface.routes }
        : {}),
      ...(baseline ? { baseline } : {}),
      signal,
      ...(parsed.rules.size > 0
...
export async function runScanCommand(
  ...
  try {
    scan = /^https?:\/\//i.test(target)
      ? await executeRemoteScan(target, {
          ...(baseline ? { baseline } : {}),
      ...
      : await executeLocalScan(path.resolve(cwd, target), {
          ...(baseline ? { baseline } : {}),
          offline: parsed.offline,
          ...(parsed.build !== undefined
            ? { build: parsed.build }
            : {}),
  ...
  if (parsed.ci) {
    const gate = evaluateSecurityGate(scan, baseline, {
      failOn:
        parsed.failOn ?? loadedConfig.config.failOn,
      maxScoreDrop:
        parsed.maxScoreDrop ??
        loadedConfig.config.maxScoreDrop,
    });
    if (!gate.passed)
      return {
        exitCode: 1,
...
export async function runCompareCommand(
  ...
  if (!previousPath || !currentPath || args.length !== 2)
    return {
      exitCode: 2,
      stderr:
        "Usage: specter compare <previous.json> <current.json>\n",
    };
  try {
    ...
    const lines = [
      ...
      ...diff.severityChanged.map(
        (change) =>
          `~ ${change.previous} → ${change.current} ${change.finding.title}`,
      ),
      "",
    ];
    const regression =
