...
import type { DependencyFinding, RuleMetadata, Severity } from "@specter/types";
...
const metadata: RuleMetadata = {
  ...
  remediation:
    "Upgrade to a patched release after validating compatibility and reviewing the advisory.",
};
...
async function readManifest(
  root: string,
): Promise<{ direct: Map<string, { version: string | undefined; dev: boolean }> }> {
...
async function parseNpmLock(
  root: string,
  direct: Map<string, { version: string | undefined; dev: boolean }>,
...
export async function inspectDependencies(root: string): Promise<DependencyInventory> {
  ...
  if (await exists(path.join(absolute, "pnpm-lock.yaml")))
    return { manager: "pnpm", dependencies: await parsePnpmLock(absolute, direct) };
  if (await exists(path.join(absolute, "package-lock.json")))
    return { manager: "npm", dependencies: await parseNpmLock(absolute, direct) };
  if (await exists(path.join(absolute, "yarn.lock")))
    return { manager: "yarn", dependencies: await parseYarnLock(absolute, direct) };
  return {
    manager: "unknown",
    dependencies: [...direct].flatMap(([name, info]) =>
      info.version ? [{ name, version: info.version, direct: true, dev: info.dev }] : [],
    ),
  };
}
...
interface OsvVulnerability {
  ...
  readonly affected?: readonly {
    readonly ranges?: readonly { readonly events?: readonly { readonly fixed?: string }[] }[];
  }[];
}
...
export class OsvProvider implements VulnerabilityProvider {
  ...
  async query(
    ...
    const body = (await response.json()) as {
      results?: readonly { vulns?: readonly OsvVulnerability[] }[];
    };
...
export class StaticAdvisoryProvider implements VulnerabilityProvider {
  ...
  async query(dependencies: readonly DependencyRecord[]): Promise<readonly Advisory[]> {
    ...
    return this.#advisories.filter((item) =>
      installed.has(`${item.package}@${item.affectedVersion}`),
    );
  }
}
...
export async function scanDependencies(
  ...
  const findings: DependencyFinding[] = advisories.map((advisory) => {
    ...
    const base = createFinding({
      ...
      remediation: advisory.patchedVersion
        ? `Upgrade ${advisory.package} to ${advisory.patchedVersion} or later after compatibility review.`
