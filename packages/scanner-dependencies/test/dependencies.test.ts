...
async function fixture(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "specter-dep-"));
  dirs.push(root);
  await writeFile(
...
describe("dependency scanner", () => {
  it("marks direct npm dependencies correctly", async () => {
    const inventory = await inspectDependencies(await fixture());
